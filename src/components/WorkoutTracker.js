  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        // User logged out — clear workouts & userId
        setUserId(null);
        setWorkouts([]);
      }
    });
  ...
  return (
    <div className="max-w-xl mx-auto mt-8 p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mt-8">
        ...
        {workouts.length === 0 && (
          <p className="text-gray-600 mt-2">No workouts logged yet.</p>
        )}
        {workouts.map(workout => (
          <div
            key={workout.id}
            className="mt-4 p-4 border rounded shadow-sm bg-gray-50"
          >
            {editingId === workout.id ? (
              ...
              <>
                <p>
                  <strong>Exercise:</strong> {workout.name}
                </p>
                <p>
